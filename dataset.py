...
class EMDataset(Dataset):
    ...
    """
    def __init__(self,root_dir: str, seed: int, category:str, transform=None, img_transform=None) -> None:
        """Initialization

        Args:
            root_dir (str): A train / validattion / test folder with images and masks subfolder.
            seed (int): A random number generator seed for the dataloader.
        ...
        if category not in allowed_categories:
            raise ValueError(f"You asked for an item of category {category} which 
                            is not one of {'/'.join(allowed_categories)}")
